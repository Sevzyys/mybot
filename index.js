...
// Triggers to moderate. Add/remove as needed.
const BANNED_WORDS = ['badword1', 'badword2']; // simple examples
...
// In-memory warning counts: { [guildId]: { [userId]: number } }
...
// Message moderation with 3-strike system
client.on('messageCreate', async (message) => {
  ...
  // DM user with purple warning embed
  ...
  // Punish based on count (timeouts use milliseconds)
  try {
    if (count === 1) {
    ...
    // missing perms, hierarchy issues, etc.
